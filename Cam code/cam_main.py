...
# Dark adaptation 
light_change = 1 # States: 1 (day, default), 2 (Dusk), 3 (Night)    
light_change_old = 1 # variable to compare to.
...
def print_time_per_frame():
    '''Used to determine the time per frame, and from that one can calculate FPS
...
def adapt_to_lighting(frame): 
    '''Enhance the frame based on average brightness. 
    \nReturns the normal or improved frame, depending on conditions'''
    # The detection algo might perceive the sudden change in brightness as "motion". It might have rapid changes in recording state and lots of short videos. 
    # This is addressed with the short video delete and if the video is long enough but there is no real object movement, the object detection will discard the video. 
    # All this will result in recordings of true movement with only objects of interest. No videos of curtains in the wind or similar.  

    # Light intensity from RGB mean, not far off from the true L of HSL. Good enough for this. 
    rgb_bright = np.mean(np.mean(np.mean(frame, axis=2), axis=1), axis=0)

    global light_change
    global min_conf

    if rgb_bright > 75: # Day light and no need to change anything
        # Keeps the image the same (i.e. brightness=0, contrast=1)
        min_conf = 0.5 # Increase to get the most probable objects that are detected
        light_change = 1
        return frame
    elif rgb_bright < 20: # Night and needs most boost 
        contrast = 3
        brightness = 50
        min_conf = 0.1 # reduce to get any objects that might be detected
        frame = cv2.addWeighted(frame, contrast, np.zeros(frame.shape, frame.dtype), 0, brightness) 
        light_change = 3
        return frame
    else: # Half bright (dusk), add some brightness and contrast 
        brightness = 40
        contrast = 2.3 
        min_conf = 0.1 # reduce to get any objects that might be detected
        frame = cv2.addWeighted(frame, contrast, np.zeros(frame.shape, frame.dtype), 0, brightness) 
...
def delay_if_light_changed(): 
    '''HALTS THE PROGRAM when the lighting conditions changed. \nSleep for 1s to prevent flickering when light is on the threshold and the algorithm can't decide.'''
    global light_change_old
    if light_change != light_change_old: 
        print('Sleeping to smooth the light change')
        time.sleep(1)
    
    light_change_old = light_change # reset to processed state
...
def get_activity_category(activity):
    '''Get the approximate object size from the amount of movement
    \nMore movement=larger obj and the other way round'''
    if activity < 0:
...
def rm_vid(video_name, reason):
    '''Remove too short video and state reason (good to see what the program is thinking)'''
...
def handle_old_recording(activity):
    '''Stop recording if old enough
    \nUses interval set by max_video_limit'''
    global recording
    if recording and time.time()-recording > max_video_limit:
        print('Stop: recording is too old')
        stop_recording(activity)

def stream_frame(frame):
    '''Send the latest frame from the camera to the server. 
    \nOr try to reconnect if something happened'''
...
def get_frames_from_vid(video_name, vid_length):
    ...
    frame2_cmd = f'ffmpeg -y -i "{video_name}" -ss 00:00:{end_time} -vframes 1 -s 640x480 "./temp/end.png" -loglevel error'
    subprocess.check_output(frame2_cmd, shell=True)

def compare_to_target_objs(video_name, vid_length, verbose=False):
    '''Returns the union of objects that are in the early and late frames of the video. 
    \nOr False if no objects are detected'''
    ...
    print(f'Detcted objects: {union_objs}')
...
def connect_to_server():
    ...
    while True:
        try:
            res = requests.get(url + f'/cam_register?state={cam_state}')
            ...
            time.sleep(1)
            # When the server restarted and the camera still runs. The cam needs to know it has to reconnect as "new"
            cam_state = 'starting'
...
def ask_for_commands():
    ...
    if last_update_request+time_step < time.time():
        # Try to get the command from the server
        try:
            global cam_state
            res = requests.get(url + f'/get_cam_commands?state={cam_state}')
        except:
...
while True:
    ...
    # Check if frame is not read correctly, normally some camera error
    if not ret:
        print('Error Cam works but no video feed. \nShutting down')
        exit()

    frame = frame.reshape((int(height), int(width), 3))
    frame = adapt_to_lighting(frame)

    delay_if_light_changed()
    ...
    if server_command == 'stream':
        ...
        process_activity()
        handle_old_recording(np.mean(activity_hist))

    # OpenCV seems to need this to run, can't work on a headless OS 
    if cv2.waitKey(1) & 0xff == ord('q'):
        break

    # print_time_per_frame()

# Kept in just to be sure
video.release()
